...
var log = require('debug-logger')('sails-sqlserver:adapter');
/**
 * sails-mssql
 *
  ...
  var adapter = {
    ...
    create: function (connection, collection, values, cb) {
      ...
      adapter.connectConnection(connection, function __CREATE__(err) {
        ...
        request.query(statement, function(err, recordsets) {
          ...
          log.debug('castValues', castValues);
          cb(err, castValues);
        });
      });
    },
    ...
    update: function (connection, collection, options, values, cb) {
      ...
      adapter.connectConnection(connection, function __UPDATE__(err) {
        ...
        request.query(statement, function(err, recordset) {
          if (err) return cb(err);
          log.debug('updating pks', recordset);
          ...
          request.query(statement, function(err, _recordset) {
            ...
            /**
              ...
              log.debug('updated models:', models);
              var castValues = [];
              var _query = new Query(connections[connection].collections[collection].definition);
